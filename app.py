...
# --- FastAnswerGenerator removed: now using hybrid_answer from rag_utils.py ---

# --- Modern Header ---
st.markdown("""
<div class='main-header'>
    <h1>💼 Enterprise RAG Assistant</h1>
    <p>Your AI-powered knowledge base for internal policies, FAQs, and more.<br>Upload documents, ask questions, and get precise, context-aware answers.</p>
</div>
...
with chat_container:
    ...
    for chat in st.session_state['chat_history']:
        ...
        # Show highlighted context(s) for transparency
        if 'contexts' in chat:
            for idx, ctx in enumerate(chat['contexts']):
                st.markdown(f'<div class="context-chunk"><em>Context {idx+1}:</em><br>{ctx}</div>', unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)

# --- Footer ---
st.markdown("""
<div class='footer'>
    &copy; 2025 Enterprise RAG Assistant &mdash; Built with Streamlit, ChromaDB, sentence-transformers, and OpenAI.<br>
    <span style='font-size:0.95em;'>For demo and academic use only.</span>
</div>
""", unsafe_allow_html=True)
...
if user_input and user_input.strip():
    ...
    if 'last_question' not in st.session_state or st.session_state['last_question'] != user_input:
        ...
        with st.spinner("🔍 Searching for answers..."):
            ...
            # Compose the chat answer
            if openai_answer:
                answer = f"**OpenAI Answer:** {openai_answer}"
            else:
                # fallback to local LLM/retrieval as before
                llm_answer = None
                try:
                    ...
                    llm_answer = None
                if llm_answer:
                    answer = f"**Extracted Answer:** {extracted_answer}\n\n**LLM Attempt:** {llm_answer}"
                else:
                    answer = f"**Extracted Answer:** {extracted_answer}"
